...
public class CPainting extends Canvas implements MouseListener {
  ...
  private Color[][][] mColors = new Color[256][256][256];
  ...
  public void paint(Graphics pGraphics) {
    ...
    int R, G, B;
      for (i = 0; i < mDimension.width; i++) {
        for (j = 0; j < mDimension.height; j++) {
        	R = (mCouleurs[i][j]>>16)&0x00FF;
        	G = (mCouleurs[i][j]>>8)&0x00FF;
        	B = mCouleurs[i][j]&0x00FF;
        	if(mColors[R][G][B] == null) {
        		mColors[R][G][B] = new Color(R,G,B);
        	}
        	pGraphics.setColor(mColors[R][G][B]);
            ...
            for (j = 0; j < 3; j++) {
              ...
              for (k = 0; k < 3; k++) {
                for (l = 0; l < 3; l++) {
                  ...
                  R += CPainting.mMatriceConv9[k][l] * ((mCouleurs[m][n]>>16) & 0x00FF);
                  G += CPainting.mMatriceConv9[k][l] * ((mCouleurs[m][n]>>8) & 0x00FF);
                  B += CPainting.mMatriceConv9[k][l] * (mCouleurs[m][n] & 0x00FF);
                }
              }
              lColor = ((int)R<<16) + ((int)G<<8) + (int)B;
              if(mColors[(int)R][(int)G][(int)B] == null) {
          		mColors[(int)R][(int)G][(int)B] = new Color(lColor);
          		}
              mGraphics.setColor(mColors[(int)R][(int)G][(int)B]);
            ...
            for (j = 0; j < 5; j++) {
              ...
              for (k = 0; k < 5; k++) {
                for (l = 0; l < 5; l++) {
                  ...
                  R += CPainting.mMatriceConv25[k][l] * ((mCouleurs[m][n]>>16) & 0x00FF);
                  G += CPainting.mMatriceConv25[k][l] * ((mCouleurs[m][n]>>8) & 0x00FF);
                  B += CPainting.mMatriceConv25[k][l] * (mCouleurs[m][n] & 0x00FF);
                }
              }
              lColor = ((int)R<<16) + ((int)G<<8) + (int)B;
              if(mColors[(int)R][(int)G][(int)B] == null) {
            ...
            for (j = 0; j < 7; j++) {
              ...
              for (k = 0; k < 7; k++) {
                for (l = 0; l < 7; l++) {
                  ...
                  R += CPainting.mMatriceConv49[k][l] * ((mCouleurs[m][n]>>16) & 0x00FF);
                  G += CPainting.mMatriceConv49[k][l] * ((mCouleurs[m][n]>>8) & 0x00FF);
                  B += CPainting.mMatriceConv49[k][l] * (mCouleurs[m][n] & 0x00FF);
                }
              }
              lColor = ((int)R<<16) + ((int)G<<8) + (int)B;
              if(mColors[(int)R][(int)G][(int)B] == null) {
